...
def populateDB(request):
    populateDatabase()
    return render(request, 'inicio.html', {'mensaje': 'Se han generado correctamente las capitales de provincia'})
    '''
...
def buscar_hotel(request):
    formulario = BusquedaHotelForm()
    hoteles = None
    
    if request.method=='POST':
        formulario = BusquedaHotelForm(request.POST)      
        if formulario.is_valid():
            #scraping
            print('Hola')
            
    return render(request, 'hotelesporfecha.html', {'formulario':formulario})
